...
public interface TreeNode<T extends TreeNode<T, ID>, ID> extends Comparable<TreeNode<T, ID>> {
   ...
   * @param function 通过父级ID获取子级对象的函数
   * @param <T> 对象类型
   * @param <ID> ID 类型
   * @return 平铺对象集合
   */
  static <T extends TreeNode<T, ID>, ID> Set<T> topDown(
  ...
  static <T extends TreeNode<T, I>, I> List<T> nest(
    ...
    Set<I> childIds = children.parallelStream()
        .map(TreeNode::getId)
        .collect(Collectors.toSet());
    Set<T> tree = topDown(childIds, nested, function);
  ...
  default void build(@NotNull Collection<T> tree) {
    ...
    children.forEach(child -> child.build(tree));
  }
}
